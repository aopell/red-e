...
namespace EBot.Commands.Modules
{
    ...
    public class HelpModule : ModuleBase<BotCommandContext>
    {
        ...
        {
            if (page <= 0) throw new CommandExecutionException("Number too low", $"{Context.User.Mention}, you can't go to {(page == 0 ? "the zeroth" : "a negative")} page");
        ...
        public async Task Help([Remainder] string command)
        {
            ...
            if (!commands.Any()) throw new CommandExecutionException("Bad argument", $"{Context.User.Mention}, there were no commands matching \"{command}\" found");
            EmbedBuilder result = new EmbedBuilder();
            result.WithTitle("Help");
            foreach (var c in commands.OrderBy(c => c.Command))
            {
